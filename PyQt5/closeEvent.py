...
class MessageBox(QtWidgets.QWidget):  # 继承自父类QtWidgets.QWidget
    def __init__(self, parent=None):  # parent = None代表此QWidget属于最上层的窗口,也就是MainWindows.
        ...
        # self.setGeometry(300, 300, 1000,1000)  # setGeometry()方法完成两个功能--设置窗口在屏幕上的位置和设置窗口本身的大小。
        # 它的前两个参数是窗口在屏幕上的x和y坐标。后两个参数是窗口本身的宽和高
        self.resize(500, 500)  # 设置窗体大小，本行可有可无。
        self.center()  # 自定义一个居中的函数
        self.setWindowTitle(u'弹窗消息事件')  # 仅仅设置窗体标题，不设置位置。
    ...
    def closeEvent(self, event):  # 函数名固定不可变
        ...
        # QtWidgets.QMessageBox.question(self,u'弹窗名/标题',u'弹窗内容',选项1,选项2)
